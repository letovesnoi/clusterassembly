# nf-core/clusterassembly: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/clusterassembly/usage](https://nf-co.re/clusterassembly/usage)
...
```console
nextflow run nf-core/clusterassembly --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/clusterassembly
```
...
First, go to the [nf-core/clusterassembly releases page](https://github.com/nf-core/clusterassembly/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
